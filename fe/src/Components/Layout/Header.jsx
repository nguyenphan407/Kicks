...
import authApi from "../../apis/authApi"; // Adjust the import path as needed

const Header = () => {
  const [visible, setVisible] = useState(false);
  const { getCartCount, token, setToken, user, cartData, cartChanged } = useContext(ShopConText);
  ...
  const logout = async () => {
    try {
      await authApi.logout(); // Call the logout function from authApi
      setToken(""); // Clear token from context
      // Optionally, clear user data from context if available
      // e.g., setUser(null);
      ...
      <div className="flex items-center xl:w-[275px] justify-end gap-2 lg:gap-10">
        ...
        <div className="relative">
          ...
          >
            ...
            <p
              className="cursor-pointer font-inter text-[10px] lg:text-[14px] uppercase font-medium flex items-center justify-between px-2 py-2 lg:px-4 lg:py-4 hover:bg-gray-100"
              onClick={() => navigate('/change-password')} // Assuming you have a change password route
            >
              Change Password
              {/* SVG Icon */}
              <svg
