...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      // Store token in local storage if "keep me logged in" is checked
      if (keepLoggedIn) {
        localStorage.setItem("token", response.data.access_token);
      }

      // Optional: Store token in memory for current session
  ...
  return (
    ...
    >
      ...
      <div className="Login flex flex-col lg:pr-[40px] w-full lg:w-[564px] mb-2 gap-6">
        ...
        <button
          type="submit"
          className="w-full bg-[#232321] flex justify-center items-center rounded-lg text-white px-4 py-[15.5px] transform transition duration-400 hover:bg-primary_blue uppercase hover:scale-[1.003] hover:text-white active:scale-[99%]"
        >
